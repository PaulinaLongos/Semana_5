...
#
# MI PREGUNTA: ¿En qué sector el ingreso aumenta más con la edad?
#
...
names(select(ingresos, starts_with("ing")))       # por cómo EMPIEZA el nombre; Se ve "ing_trabajo" "ing_capital" "ing_subsidios" "ing_total"
names(select(ingresos, where(is.numeric)))       # por el TIPO de contenido (numérico); Se ve "educ" "edad" "horas" "ing_trabajo" "ing_capital" "ing_subsidios" "ing_total"

# ✅ Deberías ver 4 columnas en el primero y 7 en el segundo.
#
# TODO: responde en un comentario.
# ¿Por qué el segundo devuelve MÁS columnas que el primero? El segundo devuelve más columnas que el primero porque el criterio de que se esta buscado es menos acotado que en el primero.
#
...
summary(casen$experiencia) # Se ve Mean: 25.78, Median: 27.00
table(casen$nivel_educ, useNA = "ifany") # Se ve Media completa:7, Sin media completa:23, Superior:30
...
resultado <- casen |>
  filter(!is.na(ingreso) & edad >= 25) |>            # ¿a quiénes necesitas? (¡ojo con los NA!)
...
resultado # Se los sectores: Educación, Servicios, Industria, Construcción, Comercio, Agricultura; Todos los anteriores con sus respectivos n e ingresos promedios.
...
nrow(filter(casen, (region == "Ñuble" | region == "Biobío") & educ > 12)) # Se ve 10
nrow(filter(casen, region == "Ñuble" | region == "Biobío" & educ > 12)) # Se ve 20
...
# ¿Algún grupo tiene n muy chico (menos de 8)? ¿Cuál? Educación con 7, Industria con 7, Construcción con 4 y Comercio con 6.
# ¿Ese promedio es confiable? ¿Qué haces al respecto? No, no es confiable ya que con tan pocas observaciones en Construcción (4) y Comercio ( 6) se puede cambiar mucho el promedio.¿Qué hacer al respecto? Aplicar el filter (n >= 8) y comprobar si cambia el ranking.
#
# TODO (si corresponde): agrega un filter(n >= 8) después del summarise y
#       compara. ¿Cambia el ranking? 

resultado_filter <- casen |>
  filter(!is.na(ingreso) & edad >= 25) |>            # ¿a quiénes necesitas? (¡ojo con los NA!)
  group_by(sector) |>          # ¿por qué grupo se parte la pregunta?
  summarise(
    n   = n(),               # NUNCA lo omitas
...
# ¿Cambia el ranking? Agregando el filter(n >=8) se ve un cambio, solo quedan los sectores: Servicios (con 13 n) y Agricultura (con 11 n).
# En la tabla sin filtrar el sector Educación era el primero con el ingreso pormedio más alto pero al exigir más evidencia deja de ser el primero y el sector Servicios pasa a ser el primero con datos más confiables.
...
#
# INTERPRETACIÓN:
# De los sectores con ingreso promedio más alto entre trabajadores con 25 años o más (considerando filter(n >=8)) primero está Servicios con $814.077 y en segundo lugar está Agricultura con $461.727.
# En la tabla sin filtrar Educación tenía el ingreso promedio más alto $908.857, pero teniendo un n=7 no se podía considerar un resultado confiable.
# El sector se asocia con el nivel de ingreso pero no significa necesariamente que trabahar en Servicios genere un ingreso más alto, pueden haber otros factores que afecten al resultado.
# En el análisis compare los niveles de ingresos entre sectores pero no mide directamente como camcia el ingreso con la edad dentro de cada sector, eso implicaria comparar grupos de edd dentro de cada uno de los sectores y calcular la correlación entre edad e ingreso por sector.
#
# LIMITACIÓN: Con el análisis no interpreto realmente el efectode la edad en sí con el ingreso, se centro más en compara los promedios por sector más que cómo varía el ingreso según la edad dentro de cada sector.
...
# No se guarda encima de data/raw/casen_reducido.csv porque si se sobreescribe el archivo original podría perder la fuente pura de datos, si alguien o yo misma necesita encima de data/raw/casen_reducido.csv porque si se sobreescribe el archivo original podría perder la fuente pura de datos, si alguien o yo misma necesita volver a partir de cero ya no podría.
